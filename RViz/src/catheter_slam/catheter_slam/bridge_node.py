...
class CatheterBridge(Node):
    def __init__(self):
        ...
        self.cmd_sub = self.create_subscription(String, 'catheter_cmd', self._cmd_callback, 10)
    ...
    def _cmd_callback(self, msg):
        ...
        elif command == 'p': self.processing_enabled = not self.processing_enabled
        ...
        while rclpy.ok():
            try:
                if self.is_tty and select.select([sys.stdin], [], [], 0)[0]:
                    ...
                    elif key == 'p': self.processing_enabled = not self.processing_enabled
                ...
                for adc_raw, enc_raw in struct.iter_unpack('>Hi', data):
                    ...
                    if self.processing_enabled:
                        # Gürültü ve mesafe visual_x_scale ile çarpıldı
                        noise = abs(np.random.normal(0, 1.0)) if self.noise_enabled else 0.0
                        self.odom_x += (delta + noise) * self.P_METRE * self.visual_x_scale
    ...
    def _publish_legend_static(self):
        ma = MarkerArray(); now = self.get_clock().now().to_msg(); step = (self.r_max - self.r_min) / 14.0
        for i in range(15):
            val, col = self.r_min + (i * step), self.colors[i]
            m_c = Marker()
            m_c.header.stamp, m_c.header.frame_id = now, "map"
            m_c.ns, m_c.id, m_c.type = "legend", i, Marker.CUBE
